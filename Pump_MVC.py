...
class Pump_Controller():
    ...
    def ImportFromFile(self, data):
        ...
        L = data[1].split()
        self.Model.FlowUnits = L[1]
        self.Model.HeadUnits = L[2]
...
class Pump_View():
    ...
    def DoPlot(self, Model):
        ...
        axes = self.ax
        axes.clear()  # Clear existing plot data
        axes.plot(headx, heady, 'r-', label='Head Fit (Quadric)')
        axes.plot(effx, effy, 'b-', label='Efficiency Fit (Cubic)')
        axes.set_xlabel('Flow')
        axes.set_ylabel('Head')
        axes.legend()
        axes.grid(True)
        axes2 = axes.twinx()
        axes2.plot(effx, effy, 'b--')
